---
# ============================================================
#  PFE defense — brag: from RPA robots to autonomous browser agents
#  Alaoui Mhamdi Hamza · ENSAH · 2025/2026
#
#  SHORT CUT (~20 slides). One slide per file under ./pages — edit there.
#  Section divider slides are inline below (SectionDivider component).
#  Palette: amber = agent path · teal = replay path.
# ============================================================
theme: default
title: brag — from RPA robots to autonomous browser agents
titleTemplate: '%s'
info: |
  ## PFE Defense — Alaoui Mhamdi Hamza
  From UiPath RPA to Autonomous AI Agents — the App Layer of Orange's
  automation migration, built at Neo Tech IT. ENSAH · 2025/2026.
author: Alaoui Mhamdi Hamza
colorSchema: auto
highlighter: shiki
lineNumbers: false
aspectRatio: 16/9
canvasWidth: 900
exportFilename: pfe-defense-brag
transition: slide-left
mdc: true
fonts:
  sans: Inter
  mono: JetBrains Mono
  weights: '300,400,600,700,800'
drawings:
  persist: false
src: ./pages/01-cover.md
---

---
src: ./pages/02-hook.md
---

---
src: ./pages/03-roadmap.md
---

---
layout: center
---

<SectionDivider :n="1" title="The company" sub="Neo Tech IT · the engagement" />

---
src: ./pages/03b-company.md
---

---
src: ./pages/04-context.md
---

---
layout: center
---

<SectionDivider :n="2" title="The problem" sub="why UiPath had to go" />

---
src: ./pages/05-problem.md
---

---
src: ./pages/06-turning-point.md
---

---
layout: center
---

<SectionDivider :n="3" title="The idea" sub="learn once, replay forever" />

---
src: ./pages/06c-idea-journey.md
---

---
src: ./pages/07-idea-two-paths.md
---

---
layout: center
---

<SectionDivider :n="4" title="How it works" sub="perception · agent · skills" />

---
src: ./pages/08-perception.md
---

---
src: ./pages/09-agent-path.md
---

---
src: ./pages/10-dsm.md
---

---
src: ./pages/11-self-healing.md
---

---
src: ./pages/12-architecture.md
---

---
src: ./pages/13-console.md
---

---
src: ./pages/13b-console-timeline.md
---

---
src: ./pages/13c-console-trace.md
---

---
src: ./pages/13a-console-instances.md
---

---
layout: center
---

<SectionDivider :n="5" title="Results" sub="what it does, what it saves" />

---
src: ./pages/14-one-task-twice.md
---

---
src: ./pages/14b-cost-curve.md
---

---
src: ./pages/16-estate-scale.md
---

---
layout: center
---

<SectionDivider :n="6" title="What's next" sub="limits & roadmap" />

---
src: ./pages/18-limits-next.md
---

---
src: ./pages/19-conclusion.md
---

---
src: ./pages/20-thanks.md
---

---
src: ./pages/01-cover.md
---
